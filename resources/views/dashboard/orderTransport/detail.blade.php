...
@section('container')
    ...
    <div class="col-xl-8 p-0 mb-4">
        <a class="btn btn-primary me-0 me-3 w-100 w-lg-auto mb-3 mb-lg-0" href="{{ route('order.index') }}">Kembali ke Data Tabel</a>
        <a class="btn btn-warning me-0 me-3 w-100 w-lg-auto mb-3 mb-lg-0" href="{{ route('order.edit', $order_transport->id) }}">Edit Data</a>
        @if (!$order_transport->acceptance_admin)
            <form action="{{ route('order.confirm.admin', $order_transport->id) }}" class="d-inline w-100 w-lg-auto me-3 mb-3 mb-lg-0" method="post">
                @csrf
                @method("put")
        ...
        @endif
        @if (!$order_transport->order_finish)
            <form action="{{ route('order.confirm.finish', $order_transport->id) }}" class="d-inline w-100 w-lg-auto mb-3 mb-lg-0" method="post">
                @csrf
                @method("put")
                <button onclick="return confirm('Konfirmasi Selesaikan Order Sewa ?')" class="btn btn-danger w-100 w-lg-auto">Selesaikan Order Sewa</button>
            </form>
        @endif
    </div>
